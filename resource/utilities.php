...
function rememberMe($user_id){
  ...
  //Cookie set to expire in about 30 dba_key_split
  setcookie("rememberUserCookie", $encryptCookieData, time()+60*60*24*100, "/");
}

/**
* checed if the cookie used is the same with the encrypted Cookie
* @param $db, database connection link
* @return bool, true if the user cookie is validate
*/
function isCookieValid($db){
  $isValid = false;

  if (isset($_COOOKIE['rememberUserCookie'])){

    /**
    * Decode cookies and extract use ID
    */
    $decryptCookieData = base64_decode($_COOKIE['rememberUserCoookie']);
    ...
    **/
    ...
    if($row = $statement->fetch()){
      ...
      /*
      * Create the user session vaiable
      */
      $_SESSION['id'] = $id;
...
function signout(){
  ...
  if(isset($_COOKIE['rememberUserCookie'])){
    unset($COOKIE['rememberUserCookie']);
...
function guard(){
  ...
  $fingerprint = md5($_SERVER['REMEOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT']);

  if((isset($_SESSION['fignerprint']) && $_SESSION['fingerprint'] != $fingerprint)){
    $isValid = false;
    signout();
